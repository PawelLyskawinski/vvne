...
layout(constant_id = 0) const bool isLeftSide = true;
...
vec4 draw_straight_lines_border(vec4 input_color, vec4 color, vec2 uv, float x_ratio)
{
  ...
  float indent_marker_line_y = 0.0f;
  if(isLeftSide)
  {
    indent_marker_line_y = (-15.0 * uv.x) + 15.0 - (2.0 * (color.a - 0.4));
  }
  else
  {
    indent_marker_line_y = (15.0 * uv.x) - (2.0 * (color.a - 0.4));
  }
