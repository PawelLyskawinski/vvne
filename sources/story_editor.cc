...
void StoryEditor::load(SDL_RWops* handle)
{
  ...
  SDL_memcpy(positions_before_grab_movement, positions, sizeof(Vec2) * entity_count);
  SDL_memset(is_selected, SDL_FALSE, sizeof(uint8_t) * entity_count);
  reset_graph_state();
}
...
void StoryEditor::imgui_update()
{
  ...
  if (is_selection_box_active())
  {
    ...
    SDL_memset(is_selected, SDL_FALSE, entity_count);
