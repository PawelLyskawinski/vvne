...
ArrayView<GuiHeightRulerText> generate_gui_height_ruler_text(struct GenerateGuiLinesCommand& cmd, Stack& allocator)
{
  ...
  for (uint32_t side = 0; side < 2; ++side)
  {
    for (int i = 0; i < 6; ++i)
    {
      ...
      GuiHeightRulerText item = {offset, 0 <= height_number ? green : red,
                                 (int)line_to_pixel_length(0.5f, cmd.screen_extent2D.height), height_number};
...
{
  float start_x_offset           = line_to_pixel_length(1.17f, cmd.screen_extent2D.width);
  float start_y_offset           = line_to_pixel_length(1.375f, cmd.screen_extent2D.height);
  ...
  for (int i = 0; i < 10; ++i)
  {
    GuiHeightRulerText item = {
                {start_x_offset,
                 start_y_offset + ((3 - i) * y_distance_between_lines) + (y_pitch_modifier * cmd.camera_y_pitch_radians)
            },
                               3 < i ? green : yellow,
                               size,
