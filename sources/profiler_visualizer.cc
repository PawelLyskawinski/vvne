                        ...
                        const float base_y_offset)
{
  ...
  const Marker*  markers_begin    = profiler.markers;
  const Marker*  markers_end      = &profiler.markers[SDL_arraysize(profiler.markers)];
  auto           lesser_begin     = [](const Marker& a, const Marker& b) { return a.begin < b.begin; };
  auto           greater_end      = [](const Marker& a, const Marker& b) { return a.end > b.end; };
  ...
  {
    for (const Marker& marker : profiler.markers)
    {
      ...
      const float marker_duration = static_cast<float>(marker.end - marker.begin);
